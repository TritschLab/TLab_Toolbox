...
digName = wsData.header.DIChannelNames;
nDig = length(digName);
for x = 1:nTraces
    tName = traceNames{x};
    tName(find(tName==' ')) = [];
...
for sweepNum = 1:nSweeps
    Ls = size(wsData.sweeps(sweepNum).acqData,1);
    ...
    if(isfield(wsData.sweeps(sweepNum),'digData'))
        digData = wsData.sweeps(sweepNum).digData;
        data.acq(sweepNum).digitalName = digName;
        for ii = 1:nDig
            data.acq(sweepNum).digitalIn{ii,1} = bitget(digData,ii);
        end
    end
end
end
